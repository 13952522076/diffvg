...
args.checkpoint = './checkpoints/%s-%s' % (
    "VanillaVAE", args.latent_dim)
if not os.path.isdir(args.checkpoint):
    mkdir_p(args.checkpoint)

SetRange = transforms.Lambda(lambda X: 2 * X - 1.)
transform_train = transforms.Compose([transforms.Resize(60), transforms.RandomHorizontalFlip(p=0.5), transforms.ToTensor(), SetRange])
transform_test = transforms.Compose([transforms.Resize(60), transforms.ToTensor(), SetRange])
