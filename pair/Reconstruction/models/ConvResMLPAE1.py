"""
Based on ConvResMLPAE, remove conv before large Conv.
"""
...
from .backbone import ConvResMLP1, MLPblock, Aff
pydiffvg.set_use_gpu(torch.cuda.is_available())


class Encoder(nn.Module):
    def __init__(self, zdim=384, pretrained=False):
        super(Encoder, self).__init__()
        net = ConvResMLP1(dim=zdim)
        self.net = net

    def forward(self, x):
        return self.net(x) # [b, 196,zdim]


class Predictor(nn.Module):
    def __init__(self, zdim=2048, paths=196, segments=2, im_size=224.0):
        super(Predictor, self).__init__()
        self.im_size = im_size
        # self.num_control_points = torch.zeros(segments, dtype=torch.int32) + 2
        self.point_predictor = nn.Sequential(
            MLPblock(dim=zdim, num_patch=paths, mlp_dim=zdim*4),
            MLPblock(dim=zdim, num_patch=paths, mlp_dim=zdim*4),
            MLPblock(dim=zdim, num_patch=paths, mlp_dim=zdim*4),
            Aff(zdim),
            nn.GELU(),
            nn.Linear(zdim, 2 * (segments * 3 + 1)),
            nn.Tanh()
        )
    ...
    def forward(self, x):  # [b,z_dim]
        ...
        return {
            "points": points,  # [b, patches, 2 * (segments * 3 + 1)]
            "colors": colors   # # [b, patches, 4]
        }
