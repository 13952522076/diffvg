"""
Based on ResNetAE, change the predictor to grouped conv.
Reason: a large vector (e.g., 2048) may introduce noise/perturbation to each paths.
As suggested by Bin Sun.
"""
import torch
...
class Predictor(nn.Module):
    def __init__(self, zdim=2048, paths=512, segments=2, im_size=224.0):
        ...
        self.point_predictor = nn.Sequential(
            nn.Conv1d(zdim, zdim, 1, groups=paths),
            nn.ReLU(inplace=True),
            nn.Conv1d(zdim, 2 * paths * (segments * 3 + 1), 1, groups=paths),
            nn.Tanh()
        )
        self.color_predictor = nn.Sequential(
            nn.Conv1d(zdim, zdim, 1, groups=paths),
            nn.ReLU(inplace=True),
            nn.Conv1d(zdim, paths*4, 1, groups=paths),  # color will be clamped to range [0,1]
            nn.Sigmoid()
        )
