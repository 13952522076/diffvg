"""
python kmeans.py demo.png --num_paths 20 --use_blob
"""
import pydiffvg
import torch
import os
...
try:
    os.makedirs("results/kmeans")
except OSError as exc:  # Python >2.5
        if exc.errno == errno.EEXIST and os.path.isdir("results/kmeans"):
            pass
        else:
            raise
...
def main():
    ...
    index_colors = torch.IntTensor(target/(2.24*2))
    ...
    for t in range(args.num_iter):
        ...
        #     pydiffvg.imwrite(img.cpu(), 'results/kmeans/{}_path_{}.png'.format(filename, args.num_paths), gamma=gamma)
        img = img[:, :, :3]
        ...
        if t == args.num_iter - 1:
            pydiffvg.save_svg('results/kmeans/{}_path_{}.svg'.format(filename, args.num_paths),
                              canvas_width, canvas_height, shapes, shape_groups)
