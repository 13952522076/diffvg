...
def biasCorrection(predict, target):
    mean_predict = predict.mean(dim=1, keepdim=True)
    mean_target = target.mean(dim=1, keepdim=True)
    uvbias = mean_target-mean_predict
    predict = predict + uvbias
    return predict
...
def main():
    ...
    for num_paths in num_paths_list:
        ...
        for t in t_range:
            ...
            img_bias = biasCorrection(img, target)
            loss = ((img_bias-target)**2).sum(dim=1, keepdim=True) # [N,1,H, W]
