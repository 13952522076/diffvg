...
python main_folder_interpolate.py --config config/interpolate.yaml --experiment experiment_exp2_8 --target_folder ../VAE_MNIST/hr_binary --save_folder VAE_MNIST
"""
import pydiffvg
import torch
import cv2
...
def plot_loss_map(pixel_loss, pool_size, savepath):
    _, _, H, W = pixel_loss.size()
    region_loss = adaptive_avg_pool2d(pixel_loss, pool_size)
    region_loss = region_loss.squeeze(dim=0).squeeze(dim=0)
    region_loss = torch.softmax(region_loss.view(-1), dim=0)\
        .reshape(pool_size, pool_size).cpu().detach().numpy()
    pixel_loss = pixel_loss.squeeze(dim=0).squeeze(dim=0)
    pixel_loss = torch.softmax(pixel_loss.view(-1), dim=0)\
        .reshape(H, W).cpu().detach().numpy()
    plt.imshow(pixel_loss, cmap='Reds')
    plt.colorbar()
    plt.savefig(savepath.replace(".png", "-loss_pixel.png"), dpi=800)
    plt.close()
    plt.imshow(region_loss, cmap='Reds')
    plt.colorbar()
    plt.savefig(savepath.replace(".png", "-loss_region.png"), dpi=800)
    plt.close()
...
def parse_args():
    ...
    parser.add_argument("--target_folder", type=str, help="target image path")
    parser.add_argument("--save_folder", type=str, help="target image path")
    parser.add_argument("--filename", type=str, help="target image path")
    parser.add_argument("--target", type=str, help="target image path")
    parser.add_argument('--log_dir', metavar='DIR', default="log/evaluate")
    ...
    cfg.target_folder = args.target_folder
...
def main(cfg):
    ...
    for path_idx, pathn in enumerate(path_schedule):
        ...
        # loss_weight = torch.nn.functional.interpolate(
        #     loss_weight, size=[h, w], mode='area')
        # loss_weight = loss_weight/(loss_weight.sum())
        # loss_weight = loss_weight.clone().detach()

        # if cfg.save.loss_plt:
        #     filename = os.path.join(
        #         cfg.experiment_dir, 'loss-heatmap-png', pathn_record_str+'.png')
        #     check_and_create_dir(filename)
        #     pixel_loss = ((x-gt)**2).sum(dim=1, keepdim=True).sqrt_() # [N, 1, H, W]
        #     plot_loss_map(pixel_loss, cfg.region_loss_pool_size, savepath=filename)
    ...
    if cfg.use_ycrcb:
        ...
        imshow = img.detach().cpu()
    pydiffvg.imwrite(imshow, filename, gamma=gamma)
    pydiffvg.save_svg(filename+".svg", w, h, shapes_record, shape_groups_record)
