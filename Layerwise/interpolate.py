...
def parse_args():
    ...
    parser.add_argument("--target2", type=str, help="target image path2")
...
if __name__ == "__main__":
    ...
    gt2 = np.array(PIL.Image.open(cfg.target2))
    gt2 = (gt2/255).astype(np.float32)
    gt2 = torch.FloatTensor(gt2).permute(2, 0, 1)[None].to(device)
    if cfg.use_ycrcb:
        gt = ycrcb_conversion(gt)
        gt2 = ycrcb_conversion(gt2)
    h, w = gt.shape[2:]
    h2, w2 = gt2.shape[2:]
    assert (h2==h and w==w2), "Please ensure two image have same resolution"
    ...
    for t in t_range:
        ...
        if cfg.use_ycrcb:
            color_reweight = torch.FloatTensor([255/219, 255/224, 255/255]).to(device)
            loss = ((x-gt2)*(color_reweight.view(1, -1, 1, 1)))**2
        else:
            loss = ((x-gt2)**2)
    ...
    if cfg.save.output:
        filename = os.path.join(
            cfg.experiment_dir, "output-svg", "interpolate.svg")
        check_and_create_dir(filename)
        pydiffvg.save_svg(filename, w, h, shapes_record, shape_groups_record)
    ...
    if cfg.save.video:
            ...
            for ii in range(0, cfg.num_iter):
                filename = os.path.join(
                    cfg.experiment_dir, "video-png",
                    "{}-intepolate{}.png".format(pathn_record_str, ii))
                img = cv2.imread(filename)
                # cv2.putText(
                #     img, "Path:{} \nIteration:{}".format(pathn_record_str, ii),
                #     (10, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
